...
module.exports = {
    create_post: async function (req, res) {
        try {
            ...
            const id_user_dir = await Teacher.findOne({
                where: {
                    id_user: id_user
                }
            })
            ...
            if (id_user_dir === null) {
                ...
                const { dataValues: { id_teacher } } = id_user_dir;
                return res.status(200).json({ success: true, data: { title: "Teacher Found", id_teacher: id_teacher } });
            }
